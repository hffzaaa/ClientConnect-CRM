        ...
        <div class="row">
            ...
            <main class="col-md-10 ms-sm-auto col-lg-10 px-md-4">
                <div class="pt-3 pb-2 mb-3 container mt-4">
                    <h2 class="mb-4">Interactions List</h2>

                    <a href="{{ route('interactions.create') }}" class="btn btn-primary mb-3">Log New Interaction</a>
                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            @foreach ($interactions as $interaction)
                                <tr>
                                    <td>
                                        <a href="{{ route('interactions.show', $interaction->id) }}" class="text-dark text-decoration-none" >
                                            {{ ucwords(strtolower($interaction->customer->name)) }}
                                        </a>
                                    </td>
                                    <td>{{ $interaction->date_time }}</td>
                                    <td>{{ ucwords(strtolower($interaction->typeinteraction)) }}</td>
                                    <td>
                                        <a href="{{ route('interactions.edit', $interaction) }}" class="btn btn-warning btn-sm">Edit</a>
                                        <form action="{{ route('interactions.destroy', $interaction) }}" method="POST" style="display:inline;">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this interaction?');">Delete</button>
                                        </form>
                                    </td>
                                </tr>
                            @endforeach
